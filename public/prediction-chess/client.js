...
(function () {
    ...
    /** Tippen ist Pflicht — ausser beim zweiten Zug eines Double Move. */
    function predictionRequired() {
        return !S.doubleSecond && !S.arrow;
    }

    function tryMove(fromR, fromC, toR, toC, move) {
        if (predictionRequired()) {
            flashError("Call your opponent's next move first — right-click drag an arrow.");
            $('predict-bar').classList.add('needed');
            setTimeout(() => $('predict-bar').classList.remove('needed'), 1200);
            return;
        }
    ...
    function renderPredictBar() {
        ...
        $('btn-clear-arrow').classList.toggle('is-hidden', !S.arrow);
        ...
        if (S.arrow) {
            ...
            txt.textContent = 'Calling ' + arrowText(S.arrow) + '.' +
                (hasSecondGuess() ? ' Draw a second arrow if you like.' : ' Now move.');
            return;
        }
        bar.classList.add('required');
        txt.textContent = 'Call their reply first — right-click drag an arrow. No call, no move.';
    }
