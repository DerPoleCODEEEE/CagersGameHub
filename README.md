...
# Handy-Layout über fünf Viewports (braucht playwright)
PORT=3113 PREDICTION_START_COINS=20 npm start &
npm run test:mobile -- http://localhost:3113
```
...
> **Offener Punkt:** `npm test` zeigt auf `test/move-gen.test.js`. Im Repo liegt
> aber `test/move-gen.js`, und das ist eine **wortgleiche Kopie von
> `public/shared/move-gen.js`** — kein Test. `npm test` läuft deshalb ins Leere;
> der Unit-Test der Zugregeln fehlt in diesem Stand.
...
Alle Bretter skalieren über eine Variable `--board-size` im jeweiligen
`style.css`. Vorher waren sie fest 600px (Quick Chess, Mutant) bzw. 560px
(Bot-Modus) breit — auf einem 390px-Schirm ragte das Brett links und rechts
aus dem Bild, und die Seite scrollte seitlich.

Die Variable steht bewusst **pro Modus** und nicht in `shared.css`: jeder Modus
rahmt sein Brett anders ein, und eine gemeinsame Regel würde stumm in einen
Modus hineinwirken, den man gerade nicht im Blick hat.

```css
--board-size: min(600px, calc(100vw - 28px));
```

Kein Höhen-Term im Grundwert — der würde das Brett auch auf einem
1280×720-Desktop schrumpfen. Das Landscape-Problem löst eine eigene Regel.

### Warum das ohne JS-Änderung funktioniert

Die Pixel-Mathematik war schon relativ: `chess/client.js` rechnet
Animationskoordinaten aus `boardEl.getBoundingClientRect().width / 8`, und die
Pfeil-Ebenen skalieren über ihre `viewBox`. Umgestellt werden mussten nur die
festen Feldmarker (`.valid-move::after` war 22px, `.capture-move::after` 65px —
auf einem 41px-Feld war der Punkt halb so groß wie das Feld). Sie rechnen jetzt
in Prozent.

### Portrait, Landscape, Reihenfolge

- **Portrait:** eine Spalte, Brett zuerst. In Prediction Chess sortiert `order`
  um — im DOM steht Chat → Brett → Shop (so gewollt für den Desktop),
  gestapelt würde der Chat sonst den ganzen ersten Bildschirm füllen. Auf dem
  Handy ist es Brett → Shop → Chat, und der Chat startet eingeklappt.
- **Landscape:** dort ist die **Höhe** der Engpass. Brett links, Bedienung
  rechts, Brettgröße aus `100dvh`. In Prediction Chess bekommt das Brett eine
  Untergrenze von 240px, weil über und unter ihm mehr UI liegt als in den
  anderen Modi — dann scrollt lieber die Seite als dass die Felder
  unbedienbar werden.
- `100dvh` statt `100vh`, damit die ein- und ausfahrende Adressleiste das
  Layout nicht wackeln lässt. Immer in einem eigenen `@supports`-Block: eine
  unbekannte Einheit macht sonst die ganze Deklaration ungültig.

### Prediction Chess war auf Touch nicht spielbar

Der Tipp-Pfeil ist Pflicht, ließ sich aber nur per **Rechtsklick-Ziehen**
zeichnen (`mousedown` mit `button === 2`). Auf Touch gibt es dafür kein
Ereignis — man konnte ziehen, kam aber nie zum „Send turn". Der im Regelbuch
beschriebene Tastaturweg war **toter Code**: `keyArrowFrom` wurde nie auf einen
Wert gesetzt.

Jetzt gilt: steht der eigene Zug, setzen **zwei Antipper** den Tipp (Startfeld,
Zielfeld). Das erste Feld wird gestrichelt markiert (`.call-from`), die
Tippzeile führt durch beide Schritte, „Take back" bricht ab. Dieselbe Eingabe
hängt am Tastatur-Enter, wodurch der dokumentierte Weg wieder funktioniert.

Die Umschaltung greift **nur auf groben Zeigern**
(`(hover: none) and (pointer: coarse)`) — mit der Maus bleibt alles wie vorher,
inklusive „Klick aufs Brett nimmt den vorgemerkten Zug zurück".

### Sonstiges

- Deko-Florks werden unter 900px ausgeblendet: sie hängen fix am unteren Rand,
  sind je 200px breit und deckten auf einem 390px-Schirm genau die Knöpfe ab.
- Der Sound-Umschalter bekommt im Portrait einen eigenen Streifen
  (`--top-strip`) und wird im Landscape auf sein Symbol eingekürzt — er saß
  vorher auf der oberen rechten Brettecke. Das `aria-label` bleibt vollständig.
- `touch-action: manipulation` auf Feldern und Knöpfen: der Browser wartet
  sonst 300ms auf einen möglichen Doppeltipp-Zoom. Pinch-Zoom bleibt erlaubt.
- Eingabefelder sind auf schmalen Schirmen mindestens 16px groß, sonst zoomt
  iOS beim Fokussieren ins Feld und gibt die Seite verschoben zurück.
- `.btn` hat jetzt `box-sizing: border-box`. Ohne das war jeder Knopf mit
  `width: 100%` um Innenabstand plus Rahmen (30px) breiter als sein Container —
  auf dem Desktop ragte „Back to Hub" sichtbar über den Kartenrand hinaus.

Abgesichert durch `npm run test:mobile` (199 Prüfungen über fünf Viewports):
kein horizontales Scrollen, Brett quadratisch und mit dem Finger treffbar,
Leisten bündig mit dem Brett, keine Überlappung durch den Sound-Knopf,
Tippziele groß genug, und eine komplette Partie Prediction Chess per Antippen.
